...
contract UniswapV2Factory is IUniswapV2Factory {
    ...
    //配对映射,token0地址=>(token1地址=>pair合约地址)
    //又或者是 token1地址=>(token0地址=>pair合约地址)
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        assembly {
            ...
            /*
                -‘0’ 表示创建新合约时的转账金额为零
                -‘add(bytecode,32)’表示合约的bytecode（字节码）的起始位置（除去前32个字节，因为前32个字节存储的是合约的长度）。
                -‘mload(bytecode)’表示装载（加载）合约的bytecode的长度，以字节为单位。
                -‘salt’表示用于给合约创建加盐的值，以增加地址的唯一性。
                加盐的目的是确保每次使用相同的合约的bytecode和salt创建合约时，生成的地址都是唯一的    
             */
