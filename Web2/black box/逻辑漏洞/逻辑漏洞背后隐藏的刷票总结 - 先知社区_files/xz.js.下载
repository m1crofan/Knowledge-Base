...
$(document).ready(function () {
    ...
    function rewardTopic() {
        const ele = $(this);
        const pk = $(this).attr('data-pk');
        // const point = $('#myModal').children('#button-value').attr('data-value');
        const point = $('#myModal').find('button.focus[data-value]').attr('data-value');
        console.log(point, 111111111);
        $.ajax({
            url: "/forum/rewarded",
            method: 'post',
            dataType: 'json',
            data: {'pk': pk, 'point': point},
            success: function (data) {
                if (data.not_authenticated) {
                    window.location.href = data.login_url
                } else {
    ...
    $('#reward_topic').bind('click', rewardTopic);
    ...
    $('#myModal').on('click', 'button[data-value]', function() {
        var $this = $(this);
        var focus = $this.hasClass('focus');
        $('#myModal').find('.focus').removeClass('focus');
        if(focus) {
